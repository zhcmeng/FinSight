...
class BaseAgent:
    ...
    async def async_run(
        ...
        """
        Agent 的主运行循环 (Think-Act-Observe 循环)
        
        流程：
        1. 检查断点恢复状态：如果开启 resume 且有历史结果，直接返回。
        2. 构建初始 Prompt：调用子类的 _prepare_init_prompt。
        3. 迭代循环：
           a. Think: LLM 思考并输出包含 XML 标签的内容。
           b. Act: 解析标签（如 <execute>），执行对应 Action（运行代码或返回最终结果）。
           c. Observe: 获取执行结果作为 Observation，更新对话上下文。
           d. Save: 每一轮迭代都会保存快照，支持随时中断和恢复。
        4. 达到终止条件（LLM 输出 final 标签）或最大迭代次数后返回结果。
        """
        # 设置日志上下文
        self.logger.set_agent_context(self.id, self.AGENT_NAME)
